      ...
      Wrap(
        ...
        children: [
          ...
          TileActionButton(
            ...
            onTap: () {
              //history에 지금 시간 추가
              history_repository.addHistory(MedicineHistory(
                //hive db에 take_date_time 저장
...
class AfterTakeTile extends StatelessWidget {
        ...
        children: [
          ...
          TileActionButton(
            ...
            title: DateFormat('HH시 mm분에')
                .format(history.history_take_time), //'20시 19분에 ',
          ),
          Text('먹었어요!', style: textStyle),
        ],
      )
  ...
  String get take_time_str => DateFormat('HH:mm')
      .format(history.history_take_time); //실 복약시간 date -> string
    ...
    showModalBottomSheet(
        ...
        builder: (context) => TimeSettingBottomSheet(
              ...
              bottom_widget: TextButton(
                autofocus: false,
                onPressed: () {
                  history_repository.deleteHistory(
                    history.key,
                  ); //복약 히스토리 삭제
      ...
      history_repository.updateHistory(
        //hive db의 takeTime 수정
...
class _MoreButton extends StatelessWidget {
                  ...
                  onPressed_update: () {
                    ...
                    Navigator.push(
                        ...
                        YaksokFadePageRoute(
                          ...
                          update_medicine_id: medicine_alarm.alarm_id,
                        ))).then(
                      (_) => Navigator.maybePop(context),
                    );
                  },
                  onPressed_delete_medicine: () {
                    //약 정보 삭제
                    //알람 삭제
                    //hive 약 데이터 삭제
                    notification.deleteMultipleAlarm(alarm_ids);
                    medicine_repository
                        .deleteMedicine(medicine_alarm.alarm_key);
                    Navigator.pop(context);
                  },
                  onPressed_delete_all: () {
                    //약 정보와 히스토리 모두삭제
                    //알람 삭제
                    //hive 히스토리 데이터 삭제
                    //hive 약 데이터 삭제
  ...
  List<String> get alarm_ids {
    final medicine = medicine_repository.medicine_box.values.singleWhere(
        (element) => element.medicine_id == medicine_alarm.alarm_id);
    final alarm_ids = medicine.medicine_alarms
        .map((alarmStr) =>
            notification.alarmId(medicine_alarm.alarm_id, alarmStr))
        .toList();
    return alarm_ids;
  }

  Iterable<int> get keys {
    final histories = history_repository.history_box.values.where((history) =>
